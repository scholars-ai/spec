...
使用 **pgmq**（Postgres 扩展，Supabase Queues 的底层实现）作为任务队列；core 内置 cron（robfig/cron）负责定时投递；不引入 Redis。
...
3. **少一个运维组件**：VPS 磁盘 66%、单人运维，每个中间件都是持续成本；Supabase 原生支持 pgmq，生产零额外部署。

...
- **自动调度**：2026-08-14 05:12（Asia/Shanghai）由 core scheduler 按 DB 配置自动投递 `topic_scout`，5 条素材批次约 50 秒完成，随后 8 条 Judge 任务全部成功；当前调度已恢复默认 `08:00/20:00`，普通定时 Scout 单 job 默认最多处理 20 条素材，手动定向投喂不受该限制。

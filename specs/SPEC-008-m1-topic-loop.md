...
- **观测可靠性修复**：`scholar-agents:1f86f57` 为 Langfuse ingestion 增加固定 3 次有限重试；agents 全量测试 `100 passed`、ruff 和 mypy 通过，并已在 VPS 本地构建部署。该重试只保护 Langfuse 观测写入，不改变业务 job 的临时错误/永久错误重试策略。
